...
func readFileWithCache(path string) ([]string, bool) {
	...
	// The path is not caller-supplied: it comes from runtime.Frame.File, i.e.
	// a source path baked in at compile time, and is already restricted to
	// .go files above. There is no traversal vector to scope with os.Root.
	// bearer:disable go_gosec_filesystem_filereadtaint
	b, err := os.ReadFile(path) // #nosec G304 -- path originates from the compiled-in stack trace
	if err != nil {
		return nil, false
	}
