...
// deepest errx.Error and extracts the specified attribute using
...
// Example usage:
//
//	code := getDeepestErrorAttribute(err, func(e Error) string {
//	  return e.code
//	})
//	// Returns the error code from the deepest error in the chain
func getDeepestErrorAttribute[T comparable](err Error, getter func(Error) T) T {
	if err.err == nil {
		return getter(err)
	}
...
// Example usage:
//
//	context := mergeNestedErrorMap(err, func(e Error) map[string]any {
//	  return e.context
//	})
//	// Returns a merged map with context from all errors in the chain,
//	// with later errors overriding earlier ones
func mergeNestedErrorMap(err Error, getter func(Error) map[string]any) map[string]any {
	if err.err == nil {
		return getter(err)
	}
