...
func TestDereferencePointers(t *testing.T) {
	...
	err := With("hello", "world").Errorf(anErrorStr).(Error)
	is.Equal(map[string]any{"hello": "world"}, err.Context())

	err = With("hello", ptr("world")).Errorf(anErrorStr).(Error)
	is.Equal(map[string]any{"hello": "world"}, err.Context())

	err = With("hello", nil).Errorf(anErrorStr).(Error)
	is.Equal(map[string]any{"hello": nil}, err.Context())

	err = With("hello", (*int)(nil)).Errorf(anErrorStr).(Error)
	is.Equal(map[string]any{"hello": nil}, err.Context())
	...
	var i **int
	err = With("hello", (***int)(&i)).Errorf(anErrorStr).(Error) //nolint:unconvert
...
func TestDereferencePointerEdgeCases(t *testing.T) {
	...
	valuePtr := &value
	mixedPtr := &valuePtr
	err = With("mixed", mixedPtr).Errorf(anErrorStr).(Error)
	is.Equal(map[string]any{"mixed": "test"}, err.Context())

	// Test with different types
	intValue := 42
	intPtr := &intValue
	err = With("int", intPtr).Errorf(anErrorStr).(Error)
	...
	err = With("struct", structPtr).Errorf(anErrorStr).(Error)
...
func TestDereferencePointerComplexTypes(t *testing.T) {
	...
	ifacePtr := &iface
	err := With("interface", ifacePtr).Errorf(anErrorStr).(Error)
	is.Equal(map[string]any{"interface": "interface_value"}, err.Context())

	// Test with array pointers
	arr := [3]int{1, 2, 3}
	arrPtr := &arr
	err = With("array", arrPtr).Errorf(anErrorStr).(Error)
	...
	nestedPtr := &nested
	err = With("nested", nestedPtr).Errorf(anErrorStr).(Error)
...
func TestDereferencePointerWithDisabled(t *testing.T) { //nolint:paralleltest
	...
	value := "test"
	ptr := &value
	err := With("test", ptr).Errorf(anErrorStr).(Error)
...
func TestDereferencePointerPanicPrevention(t *testing.T) {
	...
	err := With("very_deep", ptr11).Errorf(anErrorStr).(Error)
	is.NotNil(err)
	...
	err = With("invalid", &invalidValue).Errorf(anErrorStr).(Error)
	is.NotNil(err)
	...
	err = With("unexported", &unexported).Errorf(anErrorStr).(Error)
	is.NotNil(err)

	// Test with nil interface
	var nilInterface any
	err = With("nil_interface", &nilInterface).Errorf(anErrorStr).(Error)
	is.NotNil(err)
}
