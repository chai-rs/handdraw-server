...
/// capabilities for errx.Error instances. It includes intelligent
...
func getGOROOT() string {
	gorootOnce.Do(func() {
		...
		// Fallback to runtime.GOROOT if go env fails or returns empty
		if gorootPath == "" {
			gorootPath = runtime.GOROOT() //nolint:staticcheck
		}
	})
...
// stacktraceFrame represents a single frame in a stack trace.
...
// stacktrace represents a complete stack trace with multiple frames.
...
type stacktrace struct {
	...
	frames []stacktraceFrame // Ordered list of stack frames (most recent first)
}
...
func newStacktrace(span string) *stacktrace {
	...
	for i := 1; len(frames) < StackTraceMaxDepth; i++ {
		...
		isGoPkg := len(getGOROOT()) > 0 && strings.Contains(file, getGOROOT())
		// Only skip frames from the core errx package itself, not subpackages like errx/cmd
		// Check if the file is directly in the package directory (no subdirectory between package and filename)
		isOopsPkg := false

		if idx := strings.Index(file, packageName+"/"); idx >= 0 {
			afterPkg := file[idx+len(packageName)+1:]
			// If there's no "/" in the remainder, it's a file directly in the package (e.g., builder.go)
			// If there is a "/", it's in a subpackage (e.g., cmd/main.go)
			isOopsPkg = !strings.Contains(afterPkg, "/")
		}
...
func framesToStacktraceBlocks(blocks []lo.Tuple3[error, string, []stacktraceFrame]) []string {
	output := []string{}
...
func framesToSourceBlocks(blocks []lo.Tuple2[string, *stacktrace]) []string {
	output := [][]string{}
